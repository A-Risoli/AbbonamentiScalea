...
def validate_all_rows(
    ...
    for idx, row in enumerate(data_rows):
        ...
        is_valid, error_msg = validate_payment_method(str(method))
        if not is_valid:
            errors.append((row_num, "payment_method", error_msg))
            row_has_errors = True
        else:
            validated["payment_method"] = str(method).strip().upper()
                    ...
                    if check_period_overlap(start_i, end_i, start_j, end_j):
                        errors.append(
                            (
                                row_num_j,
                                "license_plate",
                                f"Targa duplicata con periodo sovrapposto (vedi riga {row_num_i})",
                            )
                        )
        ...
        for existing in existing_subs:
            if check_period_overlap(
                ...
                for row in data_rows:
                    if (
                        row.get("license_plate", "").strip().upper() == plate
                        and row.get("_row_number")
                    ):
                ...
                if row_num:
                    errors.append(
                        (
                            ...
                            f"Periodo: {existing['subscription_start'].strftime('%d/%m/%Y')} - "
                            f"{existing['subscription_end'].strftime('%d/%m/%Y')})",
                        )
                    )
